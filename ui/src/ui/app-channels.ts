...
export async function handleWhatsAppStart(host: MaistroApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MaistroApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MaistroApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MaistroApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MaistroApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MaistroApp): string {
...
function resolveGatewayHttpAuthHeader(host: MaistroApp): string | null {
...
function buildGatewayHttpHeaders(host: MaistroApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MaistroApp) {
...
export function handleNostrProfileToggleAdvanced(host: MaistroApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MaistroApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MaistroApp) {
  const state = host.nostrProfileFormState;
