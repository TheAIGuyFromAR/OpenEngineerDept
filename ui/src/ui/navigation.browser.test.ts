...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/maistro/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/maistro");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/maistro/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MAISTRO_CONTROL_UI_BASE_PATH__ = "/maistro";
    const app = mountApp("/maistro/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/maistro");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/maistro/sessions");
  });
