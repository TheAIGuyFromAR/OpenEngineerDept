...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `maistro-e2e-${name}-`));
  const configDir = path.join(homeDir, ".maistro");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MAISTRO_SKIP_CHANNELS: "1",
          MAISTRO_SKIP_BROWSER_CONTROL_SERVER: "1",
          MAISTRO_SKIP_CANVAS_HOST: "1",
        },
