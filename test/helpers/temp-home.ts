...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    maistroHome: process.env.MAISTRO_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MAISTRO_HOME", snapshot.maistroHome);
  restoreKey("MAISTRO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MAISTRO_HOME.
  delete process.env.MAISTRO_HOME;
  process.env.MAISTRO_STATE_DIR = path.join(base, ".maistro");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "maistro-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".maistro", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
