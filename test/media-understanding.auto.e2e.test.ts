...
import type { MaistroConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("maistro-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("maistro-bin-sherpa-");
      const modelDir = await makeTempDir("maistro-sherpa-model-");
      ...
      const cfg: MaistroConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("maistro-bin-whispercpp-");
      const modelDir = await makeTempDir("maistro-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("maistro-bin-gemini-");
      ...
      const cfg: MaistroConfig = { tools: { media: { image: {} } } };
