...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["maistroPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["maistro", "pollId"]) ??
    readNestedString(value, ["maistro", "poll", "id"]) ??
    readNestedString(value, ["data", "maistroPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "maistro", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          maistroPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "maistro poll vote",
          displayText: "Vote recorded",
          value: { maistroPollId: pollId, pollId },
        },
      },
    ],
  };
