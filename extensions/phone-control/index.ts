...
import type { MaistroPluginApi, MaistroPluginService } from "maistro/plugin-sdk";
...
function normalizeDenyList(cfg: MaistroPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MaistroPluginApi["config"]): string[] {
...
export default function register(api: MaistroPluginApi) {
  ...
  const timerService: MaistroPluginService = {
