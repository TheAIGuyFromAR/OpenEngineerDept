...
import type { MaistroPluginApi, MaistroPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<MaistroPluginApi> = {}): MaistroPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<MaistroPluginToolContext> = {}): MaistroPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: MaistroPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
