...
import { approveDevicePairing, listDevicePairing } from "maistro/plugin-sdk";
...
function resolveGatewayPort(cfg: MaistroPluginApi["config"]): number {
  const envRaw =
    process.env.MAISTRO_GATEWAY_PORT?.trim() || process.env.MAISTRO_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: MaistroPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: MaistroPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.MAISTRO_GATEWAY_TOKEN?.trim() ||
    process.env.MAISTRO_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.MAISTRO_GATEWAY_PASSWORD?.trim() ||
    process.env.MAISTRO_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: MaistroPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Maistro iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Maistro iOS app:",
            "",
            "```",
            qrAscii,
            "```",
