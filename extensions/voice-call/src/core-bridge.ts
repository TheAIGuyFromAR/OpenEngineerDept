...
function resolveMaistroRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MAISTRO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MAISTRO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMaistroRoot(), "dist", "extensionAPI.js");
