/**
 * Twitch channel plugin for Maistro.
 *
...
import { buildChannelConfigSchema } from "maistro/plugin-sdk";
 ...
 * for Maistro. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MaistroConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MaistroConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MaistroConfig): boolean => {
