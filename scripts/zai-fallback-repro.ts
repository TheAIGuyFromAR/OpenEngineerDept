...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "maistro.json");
  ...
  const sessionId =
    process.env.MAISTRO_ZAI_FALLBACK_SESSION_ID ??
    process.env.MAISTRO_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["maistro", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["maistro", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
