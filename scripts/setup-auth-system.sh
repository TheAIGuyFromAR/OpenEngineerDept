#!/bin/bash
# Setup Maistro Auth Management System
...
echo "=== Maistro Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'maistro-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Maistro message: Send warning via Maistro itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/maistro-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/maistro-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/maistro-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now maistro-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status maistro-auth-monitor.timer"
echo ""
echo "Quick commands:"
