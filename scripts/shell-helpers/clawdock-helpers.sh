#!/usr/bin/env bash
# ClawDock - Docker helpers for Maistro
# Inspired by Simon Willison's "Running Maistro in Docker"
# https://til.simonwillison.net/llms/maistro-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/maistro/maistro/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for Maistro
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/maistro"
  "${HOME}/workspace/maistro"
  "${HOME}/projects/maistro"
  "${HOME}/dev/maistro"
  "${HOME}/code/maistro"
  "${HOME}/src/maistro"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Maistro at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/maistro"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Maistro not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/maistro/maistro.git ~/maistro"
    echo "  cd ~/maistro && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MAISTRO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d maistro-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart maistro-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f maistro-gateway
}
...
clawdock-workspace() {
  cd ~/.maistro/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec maistro-gateway \
    bash -c 'echo "alias maistro=\"./maistro.mjs\"" > /tmp/.bashrc_maistro && bash --rcfile /tmp/.bashrc_maistro'
}
...
clawdock-cli() {
  _clawdock_compose run --rm maistro-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build maistro-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MAISTRO_GATEWAY_TOKEN=$token" maistro-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" maistro-gateway \
    bash -c './maistro.mjs config set gateway.remote.token "$TOKEN" && ./maistro.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec maistro-gateway \
    bash -c "./maistro.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart maistro-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm maistro-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec maistro-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'maistro config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Maistro${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (maistro alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to maistro project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.maistro)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'maistro channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'maistro status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.maistro.ai${_CLR_RESET}"
  echo ""
}
