...
const shouldBuild = (deps) => {
  if (deps.env.MAISTRO_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MAISTRO_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[maistro] ${message}\n`);
};

const runMaistro = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["maistro.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
