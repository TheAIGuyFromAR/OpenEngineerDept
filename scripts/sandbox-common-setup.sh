...
BASE_IMAGE="${BASE_IMAGE:-maistro-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-maistro-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=maistro.sandbox=1)
NOTE
