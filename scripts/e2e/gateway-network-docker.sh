...
IMAGE_NAME="maistro-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="maistro-net-e2e-$$"
GW_NAME="maistro-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MAISTRO_GATEWAY_TOKEN=$TOKEN" \
	  -e "MAISTRO_SKIP_CHANNELS=1" \
	  -e "MAISTRO_SKIP_GMAIL_WATCHER=1" \
	  -e "MAISTRO_SKIP_CRON=1" \
	  -e "MAISTRO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
