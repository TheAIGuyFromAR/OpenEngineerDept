...
ZIP=${1:?"Usage: $0 Maistro-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/maistro/maistro/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Maistro-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/maistro/maistro/releases/download/v${VERSION}/"}
