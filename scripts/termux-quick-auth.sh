...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/maistro/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Maistro Re-Auth" -c "After getting key, run: ssh $SERVER '~/maistro/scripts/mobile-reauth.sh'" --id maistro-auth
        ;;
    *)
