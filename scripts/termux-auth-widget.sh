...
# Maistro Auth Widget for Termux
...
SERVER="${MAISTRO_SERVER:-${MAISTRO_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Maistro auth..."

STATUS=$(ssh "$SERVER" '$HOME/maistro/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/maistro/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MAISTRO_EXPIRING|MAISTRO_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/maistro/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MAISTRO_EXPIRED|MAISTRO_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Maistro won't work until re-auth"
                ;;
        esac
        ;;
