...
INSTALL_URL="${MAISTRO_INSTALL_URL:-https://maistro.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MAISTRO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MAISTRO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="maistro"
PACKAGE_NAME="${MAISTRO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MAISTRO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MAISTRO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MAISTRO_INSTALL_LATEST_OUT:-}"
fi
