...
# Build and bundle Maistro into a minimal .app we can open.
# Outputs to dist/Maistro.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Maistro.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Maistro"
BUNDLE_ID="${BUNDLE_ID:-ai.maistro.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/maistro/maistro/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Maistro/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MaistroBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MaistroGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Maistro"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Maistro"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Maistro"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Maistro" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Maistro/Resources/Maistro.icns" "$APP_ROOT/Contents/Resources/Maistro.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Maistro/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MaistroKit resources"
MAISTROKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MaistroKit_MaistroKit.bundle"
if [ -d "$MAISTROKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MaistroKit_MaistroKit.bundle"
  cp -R "$MAISTROKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MaistroKit_MaistroKit.bundle"
else
  echo "WARN: MaistroKit resource bundle not found at $MAISTROKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Maistro"
killall -q Maistro 2>/dev/null || true
