...
#
# Suggested cron: */30 * * * * /home/admin/maistro/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Maistro notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., maistro-alerts)
...
STATE_FILE="$HOME/.maistro/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Maistro if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use maistro
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Maistro to $NOTIFY_PHONE..."
            maistro send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Maistro is down. Run: ssh l36 '~/maistro/scripts/mobile-reauth.sh'" "urgent"
    exit 1
