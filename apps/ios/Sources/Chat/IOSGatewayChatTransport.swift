...
struct IOSGatewayChatTransport: MaistroChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.maistro", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MaistroChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MaistroChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MaistroChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MaistroChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MaistroChatAttachmentPayload]) async throws -> MaistroChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MaistroChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MaistroChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MaistroGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MaistroGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MaistroChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MaistroAgentEventPayload.self)
                        {
