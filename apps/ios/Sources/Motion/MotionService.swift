...
final class MotionService: MotionServicing {
    func activities(params: MaistroMotionActivityParams) async throws -> MaistroMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MaistroMotionActivityEntry], Error>) in
        ...
        return MaistroMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MaistroPedometerParams) async throws -> MaistroPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<MaistroPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MaistroPedometerPayload(
