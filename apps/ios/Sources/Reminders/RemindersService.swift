...
final class RemindersService: RemindersServicing {
    func list(params: MaistroRemindersListParams) async throws -> MaistroRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MaistroReminderPayload], Error>) in
        ...
        return MaistroRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MaistroRemindersAddParams) async throws -> MaistroRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MaistroReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MaistroRemindersAddPayload(reminder: payload)
    }
