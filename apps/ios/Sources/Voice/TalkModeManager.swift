...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MAISTRO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MaistroTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MaistroTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MaistroTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MaistroTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MaistroTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MaistroTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MaistroTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MaistroTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MaistroTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MaistroTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: MaistroAgentEventPayload.self) else {
                continue
            }
