...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MaistroContactsSearchParams) async throws -> MaistroContactsSearchPayload {
        ...
        return MaistroContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MaistroContactsAddParams) async throws -> MaistroContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MaistroContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MaistroContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MaistroContactPayload {
        MaistroContactPayload(
