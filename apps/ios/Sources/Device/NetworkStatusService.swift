...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> MaistroNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> MaistroNetworkStatusPayload {
        let status: MaistroNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> MaistroNetworkStatusPayload {
        MaistroNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
