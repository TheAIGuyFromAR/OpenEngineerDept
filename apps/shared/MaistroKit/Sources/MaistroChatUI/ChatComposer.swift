...
@MainActor
struct MaistroChatComposer: View {
    @Bindable var viewModel: MaistroChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MaistroChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MaistroChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MaistroPendingAttachment.id)
                { (att: MaistroPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MaistroPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MaistroChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MaistroChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MaistroChatTheme.subtleCard)
        .clipShape(Capsule())
    }
