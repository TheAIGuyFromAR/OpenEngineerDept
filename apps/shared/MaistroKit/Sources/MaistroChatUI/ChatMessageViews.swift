...
struct ChatMessageBubble: View {
    let message: MaistroChatMessage
    let style: MaistroChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MaistroChatTheme.userText : MaistroChatTheme.assistantText
    ...
    private var inlineAttachments: [MaistroChatMessageContent] {
    ...
    private var toolCalls: [MaistroChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MaistroChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MaistroChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MaistroChatTheme.onboardingAssistantBubble
        }
        return MaistroChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MaistroChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MaistroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MaistroChatTheme.userText : MaistroChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MaistroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MaistroChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MaistroChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MaistroChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MaistroChatTheme.assistantText)
            }
        }
    }
}
