...
private let chatUILogger = Logger(subsystem: "ai.maistro", category: "MaistroChatUI")

@MainActor
@Observable
public final class MaistroChatViewModel {
    public private(set) var messages: [MaistroChatMessage] = []
    ...
    public var attachments: [MaistroPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MaistroChatPendingToolCall] = []
    public private(set) var sessions: [MaistroChatSessionEntry] = []
    private let transport: any MaistroChatTransport
    ...
    private var pendingToolCallsById: [String: MaistroChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MaistroChatTransport) {
    ...
    public var sessionChoices: [MaistroChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [MaistroChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MaistroPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MaistroChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MaistroChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: MaistroChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MaistroChatMessage],
        incoming: [MaistroChatMessage]) -> [MaistroChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MaistroChatMessage]) -> [MaistroChatMessage] {
        var result: [MaistroChatMessage] = []
    ...
    private static func dedupeKey(for message: MaistroChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MaistroChatMessageContent] = [
            MaistroChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MaistroChatAttachmentPayload in
            MaistroChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MaistroChatSessionEntry {
        MaistroChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MaistroChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MaistroChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MaistroAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MaistroChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MaistroPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
