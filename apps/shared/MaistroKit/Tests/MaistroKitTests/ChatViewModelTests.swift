...
private final class TestChatTransport: @unchecked Sendable, MaistroChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MaistroChatHistoryPayload]
    private let sessionsResponses: [MaistroChatSessionsListResponse]

    private let stream: AsyncStream<MaistroChatTransportEvent>
    private let continuation: AsyncStream<MaistroChatTransportEvent>.Continuation

    init(
        historyResponses: [MaistroChatHistoryPayload],
        sessionsResponses: [MaistroChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MaistroChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MaistroChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MaistroChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [MaistroChatAttachmentPayload]) async throws -> MaistroChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return MaistroChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MaistroChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MaistroChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MaistroChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MaistroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MaistroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MaistroChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MaistroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MaistroChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MaistroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MaistroChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
