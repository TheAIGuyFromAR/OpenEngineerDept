...
struct ConnectOptions {
    ...
    var clientId: String = "maistro-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          maistro-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: maistro-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Maistro macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Maistro macOS Gateway Connect")
