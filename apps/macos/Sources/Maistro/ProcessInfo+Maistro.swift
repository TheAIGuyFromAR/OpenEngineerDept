...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.maistro.mac`) even if the shipped
    ...
    {
        if environment["MAISTRO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "maistro.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "maistro.nixMode") { return true }
