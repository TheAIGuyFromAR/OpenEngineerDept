...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MaistroProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.maistro", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MaistroKit.AnyCodable]()) {
                $0[$1.key] = MaistroKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MaistroProtocol.AnyCodable?) -> [String: MaistroProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MaistroProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MaistroKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MaistroProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("maistro.control.heartbeat")
    static let controlAgentEvent = Notification.Name("maistro.control.agent")
}
