...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.maistro", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MaistroSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MaistroSessionPreviewEntry,
    ...
    private static func cache(payload: MaistroSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MaistroChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MaistroKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MaistroKit.AnyCodable]) -> [MaistroChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MaistroChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MaistroChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MaistroChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MaistroChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MaistroChatMessage) -> String? {
