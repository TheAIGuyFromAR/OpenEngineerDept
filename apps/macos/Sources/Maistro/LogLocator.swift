...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MAISTRO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/maistro")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("maistro-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("maistro-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/maistro/ (rolling or stdout), or nil if none exist.
