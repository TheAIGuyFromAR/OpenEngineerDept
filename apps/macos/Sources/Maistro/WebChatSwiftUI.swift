...
private let webChatSwiftLogger = Logger(subsystem: "ai.maistro", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MaistroChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MaistroChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MaistroChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MaistroChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MaistroGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MaistroChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MaistroChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MaistroChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MaistroChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MaistroChatView>) -> NSViewController
    {
