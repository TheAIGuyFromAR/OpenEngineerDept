...
enum CommandResolver {
    private static let projectRootDefaultsKey = "maistro.gatewayProjectRootPath"
    private static let helperName = "maistro"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let maistroEntry = root.appendingPathComponent("maistro.mjs").path
        if FileManager().isReadableFile(atPath: maistroEntry) { return maistroEntry }
        let binEntry = root.appendingPathComponent("bin/maistro.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/maistro")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let maistroPaths = self.maistroManagedPaths(home: home)
        if !maistroPaths.isEmpty {
            extras.insert(contentsOf: maistroPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + maistroPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func maistroManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".maistro"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func maistroExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMaistroExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("maistro.mjs").path,
            root.appendingPathComponent("bin/maistro.js").path,
        ]
    ...
    static func hasAnyMaistroInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.maistroExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func maistroNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let maistroPath = self.projectMaistroExecutable(projectRoot: root) {
                return [maistroPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "maistro", subcommand] + extraArgs
            }
            if let maistroPath = self.maistroExecutable(searchPaths: searchPaths) {
                return [maistroPath, subcommand] + extraArgs
            }

            let missingEntry = """
            maistro entrypoint missing (looked for dist/index.js or maistro.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real maistro CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/maistro"
        ...
        \(cliSection)
        \(projectSection)
        if command -v maistro >/dev/null 2>&1; then
          CLI="$(command -v maistro)"
          maistro \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/maistro.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/maistro.mjs"
            node "$PRJ/maistro.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/maistro.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/maistro.js"
            node "$PRJ/bin/maistro.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent maistro"
          pnpm --silent maistro \(quotedArgs);
        else
          echo "maistro CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MaistroConfigFile.loadDict()
