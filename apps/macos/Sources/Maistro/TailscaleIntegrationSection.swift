...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.maistro/maistro.json. Prefer MAISTRO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.maistro/maistro.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.maistro/maistro.json. Restart the gateway to apply."
        }
