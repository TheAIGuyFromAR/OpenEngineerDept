...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.maistro", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Maistro run already has an SSH listener on the expected port (common after restarts),
