...
enum DebugActions {
    private static let verboseDefaultsKey = "maistro.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "maistro.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MaistroPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Maistro", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MaistroPaths.configURL
        guard
