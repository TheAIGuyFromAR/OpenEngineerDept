...
enum MaistroLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MaistroOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MaistroFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MaistroLogging.bootstrapIfNeeded()
        let label = MaistroLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MaistroOSLogHandler: LogHandler {
...
struct MaistroFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MaistroLogging.parseLabel(self.label)
