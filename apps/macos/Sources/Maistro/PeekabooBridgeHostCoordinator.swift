...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.maistro", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MaistroPeekabooBridgeServices?
    private static var maistroSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Maistro", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MaistroPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.maistroSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.maistroSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MaistroPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.maistro.peekaboo")
