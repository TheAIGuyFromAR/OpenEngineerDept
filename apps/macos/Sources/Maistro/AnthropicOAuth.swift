...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Maistro token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: MaistroOAuthStore.AnthropicOAuthStatus = MaistroOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.maistro", category: "anthropic-oauth")
...
enum MaistroOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let maistroOAuthDirEnv = "MAISTRO_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Maistro OAuth token file not found"
            case .unreadableFile: "Maistro OAuth token file not readable"
            case .invalidJSON: "Maistro OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Maistro OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Maistro OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.maistroOAuthDirEnv]?
