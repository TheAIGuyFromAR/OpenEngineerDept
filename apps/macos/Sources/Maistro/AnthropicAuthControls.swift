...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MaistroOAuthStore.AnthropicOAuthStatus = MaistroOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MaistroOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MaistroOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MaistroOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try MaistroOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Maistro can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MaistroOAuthStore.AnthropicOAuthStatus,
