...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MaistroProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MaistroProtocol.AnyCodable] = [
            "presence": .init([MaistroProtocol.AnyCodable(entry)]),
        ]
        let payload = MaistroProtocol.AnyCodable(payloadMap)
