...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["maistro"]
        let text = "hey maistro"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey maistro do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["maistro"], minPostTriggerGap: 0.3)
