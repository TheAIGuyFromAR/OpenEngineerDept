...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "maistro-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MAISTRO_GATEWAY_TOKEN: "test-token",
    MAISTRO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MAISTRO_WORKSPACE_DIR: join(sandbox.rootDir, "maistro"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MAISTRO_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MAISTRO_EXTRA_MOUNTS: undefined,
        MAISTRO_HOME_VOLUME: "maistro-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MAISTRO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MAISTRO_EXTRA_MOUNTS=");
    expect(envFile).toContain("MAISTRO_HOME_VOLUME=maistro-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("maistro-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("maistro-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MAISTRO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
