...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MAISTRO_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Maistro Canvas");
    } finally {
      await handler.close();
