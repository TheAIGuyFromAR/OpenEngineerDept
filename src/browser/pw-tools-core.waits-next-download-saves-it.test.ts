...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMaistroTmpDir: vi.fn(() => "/tmp/maistro"),
}));
vi.mock("../infra/tmp-maistro-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMaistroTmpDir.mockReturnValue("/tmp/maistro");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMaistroTmpDir.mockReturnValue("/tmp/maistro-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "maistro-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMaistroTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "maistro-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "maistro-preferred", "downloads")}${path.sep}`),
    );
  });
