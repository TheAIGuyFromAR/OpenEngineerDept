...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".maistro-profile-decorated");
}
 ...
 */
export function decorateMaistroProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MAISTRO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MAISTRO_BROWSER_COLOR).toUpperCase();
