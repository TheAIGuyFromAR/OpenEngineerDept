import type { BrowserConfig, BrowserProfileConfig, MaistroConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_MAISTRO_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "maistro" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MAISTRO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "maistro" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MAISTRO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MAISTRO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Maistro browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for maistro/maistro CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MaistroConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MAISTRO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "maistro";
