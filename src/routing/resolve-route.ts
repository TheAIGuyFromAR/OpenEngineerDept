...
function listAgents(cfg: MaistroConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MaistroConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MaistroConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MaistroConfig, EvaluatedBindingsCache>();
