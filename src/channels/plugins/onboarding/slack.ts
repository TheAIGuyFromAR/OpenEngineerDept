...
function setSlackDmPolicy(cfg: MaistroConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Maistro";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Maistro`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/maistro",
          description: "Send a message to Maistro",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: MaistroConfig, allowFrom: string[]): MaistroConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
