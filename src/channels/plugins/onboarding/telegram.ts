...
function setTelegramDmPolicy(cfg: MaistroConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("maistro logs --follow")}\` (safest)`,
