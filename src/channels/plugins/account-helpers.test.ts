...
function cfg(accounts?: Record<string, unknown> | null): MaistroConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MaistroConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MaistroConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MaistroConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MaistroConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MaistroConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MaistroConfig)).toBe("default");
    });
  });
});
