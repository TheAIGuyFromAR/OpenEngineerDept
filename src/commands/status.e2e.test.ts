...
beforeAll(() => {
  envSnapshot = captureEnv(["MAISTRO_PROFILE"]);
  process.env.MAISTRO_PROFILE = "isolated";
});
...
vi.mock("../infra/maistro-root.js", () => ({
  resolveMaistroPackageRoot: vi.fn().mockResolvedValue("/tmp/maistro"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/maistro/pnpm-lock.yaml",
      markerPath: "/tmp/maistro/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Maistro status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("maistro status --all") ||
          l.includes("maistro --profile isolated status --all") ||
          l.includes("maistro status --all") ||
          l.includes("maistro --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MAISTRO_GATEWAY_TOKEN = "abcd1234";
    try {
