...
const SEARCH_TOOL = "https://docs.maistro.ai/mcp.SearchMaistro";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.maistro.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('maistro docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.maistro.ai/");
      runtime.log(`Search: ${formatCliCommand('maistro docs "your query"')}`);
    }
    return;
  }
