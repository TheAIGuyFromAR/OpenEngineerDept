...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMaistroAgentDir: vi.fn().mockReturnValue("/tmp/maistro-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/maistro-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMaistroAgentDir: mocks.resolveMaistroAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/maistro-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMaistroAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/maistro-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/maistro-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/maistro-agent-custom");
