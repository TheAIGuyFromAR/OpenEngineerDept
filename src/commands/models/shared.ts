...
export async function updateConfig(
  mutator: (cfg: MaistroConfig) => MaistroConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MaistroConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MaistroConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("maistro agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
