...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMaistroAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("maistro models auth setup-token")}\` or \`${formatCliCommand("maistro configure")}\`.`
          : `Run \`${formatCliCommand("maistro configure")}\` or set an API key env var.`;
