...
function resolveMode(cfg: MaistroConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Maistro doctor");

  const root = await resolveMaistroPackageRoot({
  ...
  let cfg: MaistroConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("maistro configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("maistro config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("maistro setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("maistro doctor --fix")}" to apply changes.`);
  }
