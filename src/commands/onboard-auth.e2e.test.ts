...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MAISTRO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("maistro-oauth-");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MAISTRO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("maistro-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
