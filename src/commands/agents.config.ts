...
export function listAgentEntries(cfg: MaistroConfig): AgentEntry[] {
...
function resolveAgentName(cfg: MaistroConfig, agentId: string) {
...
function resolveAgentModel(cfg: MaistroConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MaistroConfig): AgentSummary[] {
