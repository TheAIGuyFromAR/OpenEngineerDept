...
async function withOnboardEnv(
  ...
  const prev = captureEnv([
    ...
    "MAISTRO_STATE_DIR",
    "MAISTRO_CONFIG_PATH",
    "MAISTRO_SKIP_CHANNELS",
    "MAISTRO_SKIP_GMAIL_WATCHER",
    "MAISTRO_SKIP_CRON",
    "MAISTRO_SKIP_CANVAS_HOST",
    "MAISTRO_GATEWAY_TOKEN",
    "MAISTRO_GATEWAY_PASSWORD",
    "CUSTOM_API_KEY",
    "MAISTRO_DISABLE_CONFIG_CACHE",
  ]);
  ...
  process.env.MAISTRO_DISABLE_CONFIG_CACHE = "1";
  delete process.env.MAISTRO_GATEWAY_TOKEN;
  delete process.env.MAISTRO_GATEWAY_PASSWORD;
  delete process.env.CUSTOM_API_KEY;

  const tempHome = await makeTempWorkspace(prefix);
  const configPath = path.join(tempHome, "maistro.json");
  process.env.HOME = tempHome;
  process.env.MAISTRO_STATE_DIR = tempHome;
  process.env.MAISTRO_CONFIG_PATH = configPath;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("maistro-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("maistro-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("maistro-onboard-zai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("maistro-onboard-zai-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("maistro-onboard-xai-", async (env) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("maistro-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("maistro-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("maistro-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("maistro-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("maistro-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "maistro-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "maistro-onboard-cf-gateway-infer-",
      options: {},
    },
  ])(
    "$name",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("maistro-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("maistro-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("maistro-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveOnboardingWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "maistro-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "maistro-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "maistro-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "maistro-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            authChoice: "custom-api-key",
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("maistro-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "maistro-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            customApiKey: "custom-test-key",
