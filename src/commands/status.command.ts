...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Maistro status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("maistro security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("maistro security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.maistro.ai/faq");
  runtime.log("Troubleshooting: https://docs.maistro.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("maistro status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("maistro logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("maistro status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("maistro gateway probe")}`);
  }
}
