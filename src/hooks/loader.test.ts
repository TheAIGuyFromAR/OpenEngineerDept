...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.MAISTRO_BUNDLED_HOOKS_DIR;
    process.env.MAISTRO_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.MAISTRO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MAISTRO_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
