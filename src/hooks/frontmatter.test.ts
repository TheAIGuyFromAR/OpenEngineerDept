...
`;
    ...
    expect(parsed.maistro.emoji).toBe("💾");
    expect(parsed.maistro.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.maistro.emoji).toBe("📝");
    expect(parsed.maistro.events).toEqual(["command"]);
    expect(parsed.maistro.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.maistro.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"maistro": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"maistro": {"events": ["test"]}}');
  });
...
describe("resolveMaistroMetadata", () => {
  it("extracts maistro metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMaistroMetadata(frontmatter);
  ...
  it("returns undefined when maistro key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        maistro: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Maistro" },
            { id: "npm", kind: "npm", package: "@maistro/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@maistro/hook");
  });
...
homepage: https://docs.maistro.ai/automation/hooks#session-memory
metadata:
  {
    "maistro":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Maistro" }],
      },
  }
---
...
`;
    ...
    const maistro = resolveMaistroMetadata(frontmatter);
    expect(maistro).toBeDefined();
    expect(maistro?.emoji).toBe("💾");
    expect(maistro?.events).toEqual(["command:new"]);
    expect(maistro?.requires?.config).toEqual(["workspace.dir"]);
    expect(maistro?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const maistro = resolveMaistroMetadata(frontmatter);
    expect(maistro?.emoji).toBe("disk");
    expect(maistro?.events).toEqual(["command:new"]);
  });
});
