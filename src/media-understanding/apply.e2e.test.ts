...
function createGroqAudioConfig(): MaistroConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MaistroConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function createTempMediaFile(params: { fileName: string; content: Buffer | string }) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-media-"));
