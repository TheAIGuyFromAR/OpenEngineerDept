...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".maistro");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MAISTRO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MAISTRO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MAISTRO_STATE_DIR.
