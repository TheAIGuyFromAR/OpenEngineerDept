...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `maistro-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        maistro: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      maistro: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.maistro/extensions and uses unscoped id", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
  ...
  it("rejects packages without maistro.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@maistro/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("maistro.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@maistro/voice-call@0.0.1", "--ignore-scripts"]);
