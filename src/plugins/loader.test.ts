...
const fixtureRoot = path.join(os.tmpdir(), `maistro-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MAISTRO_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "maistro.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MAISTRO_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMaistroPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.MAISTRO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMaistroPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadMaistroPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.MAISTRO_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
