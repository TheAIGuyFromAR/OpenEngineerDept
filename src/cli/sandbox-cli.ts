...
const SANDBOX_EXAMPLES = {
  main: [
    ["maistro sandbox list", "List all sandbox containers."],
    ["maistro sandbox list --browser", "List only browser containers."],
    ["maistro sandbox recreate --all", "Recreate all containers."],
    ["maistro sandbox recreate --session main", "Recreate a specific session."],
    ["maistro sandbox recreate --agent mybot", "Recreate agent containers."],
    ["maistro sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["maistro sandbox list", "List all sandbox containers."],
    ["maistro sandbox list --browser", "List only browser containers."],
    ["maistro sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["maistro sandbox recreate --all", "Recreate all containers."],
    ["maistro sandbox recreate --session main", "Recreate a specific session."],
    ["maistro sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["maistro sandbox recreate --browser --all", "Recreate only browser containers."],
    ["maistro sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["maistro sandbox explain", "Show effective sandbox config."],
    ["maistro sandbox explain --session agent:main:main", "Explain a specific session."],
    ["maistro sandbox explain --agent work", "Explain an agent sandbox."],
    ["maistro sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.maistro.ai/cli/sandbox")}\n`,
    )
    .action(() => {
