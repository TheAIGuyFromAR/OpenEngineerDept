    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maistro nodes status", "List known nodes with live status."],
          ["maistro nodes pairing pending", "Show pending node pairing requests."],
          ['maistro nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["maistro nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.maistro.ai/cli/nodes")}\n`,
    );
