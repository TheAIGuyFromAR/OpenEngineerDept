...
vi.mock("../infra/maistro-root.js", () => ({
  resolveMaistroPackageRoot: vi.fn(),
}));
...
const { resolveMaistroPackageRoot } = await import("../infra/maistro-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("maistro-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveMaistroPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMaistroPackageRoot).mockReset();
    ...
    vi.mocked(resolveMaistroPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("maistro");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/maistro-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Maistro update status");
  });
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["MAISTRO_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.MAISTRO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("maistro-update-wizard");
    const envSnapshot = captureEnv(["MAISTRO_GIT_DIR"]);
    try {
      setTty(true);
      process.env.MAISTRO_GIT_DIR = tempDir;
