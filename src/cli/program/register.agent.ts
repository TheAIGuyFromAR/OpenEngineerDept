...
${formatHelpExamples([
  ['maistro agent --to +15555550123 --message "status update"', "Start a new session."],
  ['maistro agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'maistro agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'maistro agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['maistro agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'maistro agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.maistro.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.maistro.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['maistro agents set-identity --agent main --name "Maistro" --emoji "🦞"', "Set name + emoji."],
  ["maistro agents set-identity --agent main --avatar avatars/maistro.png", "Set avatar path."],
  [
    "maistro agents set-identity --workspace ~/.maistro/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "maistro agents set-identity --identity-file ~/.maistro/workspace/IDENTITY.md --agent main",
