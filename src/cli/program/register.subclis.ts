...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.MAISTRO_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.MAISTRO_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<MaistroConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Maistro docs",
    hasSubcommands: false,
    register: async (program) => {
