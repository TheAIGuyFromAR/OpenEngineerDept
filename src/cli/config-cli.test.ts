...
import type { ConfigFileSnapshot, MaistroConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `maistro config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: MaistroConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: MaistroConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: MaistroConfig, config: MaistroConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: MaistroConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
