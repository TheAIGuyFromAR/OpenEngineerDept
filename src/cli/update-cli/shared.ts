...
import { resolveMaistroPackageRoot } from "../../infra/maistro-root.js";
...
const MAISTRO_REPO_URL = "https://github.com/maistro/maistro.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "maistro";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("maistro@")) {
    return trimmed.slice("maistro@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.MAISTRO_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveMaistroPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MAISTRO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MAISTRO_GIT_DIR points at a non-git directory: ${params.dir}. Set MAISTRO_GIT_DIR to an empty folder or an maistro checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MAISTRO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "maistro.mjs");
