...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: MaistroConfig; warnings: string[] } {
...
function enablePluginInConfig(config: MaistroConfig, pluginId: string): MaistroConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Maistro plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.maistro.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: MaistroConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.maistro.ai/plugin");
      lines.push("");
