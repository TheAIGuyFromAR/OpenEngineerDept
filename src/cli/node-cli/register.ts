    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "maistro node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["maistro node status", "Check node host service status."],
          ["maistro node install", "Install the node host service."],
          ["maistro node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.maistro.ai/cli/node")}\n`,
    );
