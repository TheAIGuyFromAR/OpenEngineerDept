...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("maistro node install"), formatCliCommand("maistro node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("maistro node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MAISTRO_LOG_PREFIX: baseEnv.MAISTRO_LOG_PREFIX ?? "node",
