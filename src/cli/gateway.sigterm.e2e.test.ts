...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      MAISTRO_NO_RESPAWN: "1",
      MAISTRO_STATE_DIR: stateDir,
      MAISTRO_SKIP_CHANNELS: "1",
      MAISTRO_SKIP_GMAIL_WATCHER: "1",
      MAISTRO_SKIP_CRON: "1",
      MAISTRO_SKIP_BROWSER_CONTROL_SERVER: "1",
      MAISTRO_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "maistro-entry-bootstrap.mjs");
