...
describe("skills-cli", () => {
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["MAISTRO_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-bundled-skills-test-"));
      process.env.MAISTRO_BUNDLED_SKILLS_DIR = tempBundledDir;
