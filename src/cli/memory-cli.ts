    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maistro memory status", "Show index and provider status."],
          ["maistro memory index --force", "Force a full reindex."],
          ['maistro memory search --query "deployment notes"', "Search indexed memory entries."],
          ["maistro memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.maistro.ai/cli/memory")}\n`,
    );
