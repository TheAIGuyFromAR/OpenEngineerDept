...
function resolveGatewayPort(cfg: MaistroConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: MaistroConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.MAISTRO_GATEWAY_PASSWORD?.trim() ||
    env.MAISTRO_GATEWAY_PASSWORD?.trim() ||
