...
} from "./maistro-tools.subagents.test-harness.js";
import "./test-helpers/fast-core-tools.js";

let createMaistroTools: (typeof import("./maistro-tools.js"))["createMaistroTools"];
...
describe("maistro-tools: subagents steer failure", () => {
  beforeEach(async () => {
    vi.resetModules();
    ({ createMaistroTools } = await import("./maistro-tools.js"));
    ...
    const storePath = path.join(
      os.tmpdir(),
      `maistro-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
