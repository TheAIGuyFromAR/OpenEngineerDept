...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("maistro-ws-", async (workspaceDir) => {
      await withTempDir("maistro-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMaistroCodingTools({ workspaceDir });
    ...
    await withTempDir("maistro-ws-", async (workspaceDir) => {
      await withTempDir("maistro-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello maistro");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("maistro-ws-", async (workspaceDir) => {
      const tools = createMaistroCodingTools({
        workspaceDir,
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("maistro-ws-", async (workspaceDir) => {
      await withTempDir("maistro-override-", async (overrideDir) => {
        const tools = createMaistroCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("maistro-sandbox-", async (sandboxDir) => {
      await withTempDir("maistro-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "maistro-sbx-test",
          containerWorkdir: "/workspace",
          fsBridge: createHostSandboxFsBridge(sandboxDir),
          docker: {
            image: "maistro-sandbox:bookworm-slim",
            containerPrefix: "maistro-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createMaistroCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
