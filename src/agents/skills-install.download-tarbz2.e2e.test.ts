...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-skills-install-"));
  try {
...
function restoreMaistroStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.MAISTRO_STATE_DIR;
    return;
  }
  process.env.MAISTRO_STATE_DIR = originalValue;
}

const originalStateDir = process.env.MAISTRO_STATE_DIR;

afterEach(() => {
  restoreMaistroStateDir(originalStateDir);
});
