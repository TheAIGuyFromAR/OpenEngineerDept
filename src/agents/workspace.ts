...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MAISTRO_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".maistro", `workspace-${profile}`);
  }
  return path.join(home, ".maistro", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".maistro";
