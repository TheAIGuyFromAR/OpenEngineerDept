...
} from "./maistro-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateMaistroTools = (typeof import("./maistro-tools.js"))["createMaistroTools"];
type CreateMaistroToolsOpts = Parameters<CreateMaistroTools>[0];

async function getSessionsSpawnTool(opts: CreateMaistroToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createMaistroTools } = await import("./maistro-tools.js");
  const tool = createMaistroTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("maistro-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
