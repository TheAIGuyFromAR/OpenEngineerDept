...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MAISTRO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MAISTRO_HOME: "/srv/maistro-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/maistro-home"), ".maistro", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".maistro", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("maistro-workspace-");
