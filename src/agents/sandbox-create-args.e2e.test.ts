...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "maistro-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "maistro-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "maistro.sandbox=1",
        "--label",
        "maistro.sessionKey=main",
        "--label",
        "maistro.createdAtMs=1700000000000",
        "--label",
        "maistro.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
  ...
  it("throws on dangerous bind mounts (parent path)", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "maistro-sbx-dangerous-parent",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
      }),
