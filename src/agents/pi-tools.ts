...
function resolveExecConfig(params: { cfg?: MaistroConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: MaistroConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createMaistroCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createMaistroReadTool(freshReadTool);
