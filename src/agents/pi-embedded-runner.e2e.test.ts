...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-embedded-agent-"));
...
const ensureModels = (cfg: MaistroConfig) => ensureMaistroModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("persists prompt transport errors as transcript entries", async () => {
    ...
    const promptErrorEntry = entries.find(
      (entry) => entry.type === "custom" && entry.customType === "maistro:prompt-error",
