...
import { createMaistroTools } from "./maistro-tools.js";
import { __testing, createMaistroCodingTools } from "./pi-tools.js";
...
describe("createMaistroCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createMaistroTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createMaistroCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "maistro-outside.txt");
