...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-pi-tools-"));
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createMaistroCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createMaistroCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createMaistroCodingTools({
      config: cfg,
    ...
    const defaultTools = createMaistroCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createMaistroCodingTools({
      config: cfg,
    ...
    const bobTools = createMaistroCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createMaistroCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createMaistroCodingTools({
      config: cfg,
