...
type WebSearchConfig = NonNullable<MaistroConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MaistroConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("maistro configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.maistro.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://maistro.ai",
      "X-Title": "Maistro Web Search",
    },
