...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("maistro-triggers-", async (home) => {
      const inboundDir = join(home, ".maistro", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "maistro"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("maistro-triggers-bypass-", async (home) => {
      // Sensitive host file outside .maistro
