...
function createReplyConfig(home: string, streamMode?: "block"): MaistroConfig {
  return {
    agents: {
      defaults: {
...
describe("block streaming", () => {
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-stream-"));
  });
  ...
  beforeEach(() => {
    vi.stubEnv("MAISTRO_TEST_FAST", "1");
