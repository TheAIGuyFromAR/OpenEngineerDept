...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { MAISTRO_STATE_DIR: process.env.MAISTRO_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.MAISTRO_STATE_DIR === undefined) {
      delete process.env.MAISTRO_STATE_DIR;
    } else {
      process.env.MAISTRO_STATE_DIR = snapshot.MAISTRO_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "maistro-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "maistro-memory-flush-"));
  });
