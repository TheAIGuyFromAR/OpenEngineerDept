...
import { scheduleGatewaySigusr1Restart, triggerMaistroRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Maistro in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMaistroRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Maistro via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
