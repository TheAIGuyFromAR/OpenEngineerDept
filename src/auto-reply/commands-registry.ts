...
export function isCommandEnabled(cfg: MaistroConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MaistroConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MaistroConfig),
...
export function getCommandDetection(_cfg?: MaistroConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MaistroConfig) {
