...
import { resolvePreferredMaistroTmpDir } from "../infra/tmp-maistro-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredMaistroTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "maistro.log"); // legacy single-file path

const LOG_PREFIX = "maistro";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MaistroConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => MaistroConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MAISTRO_TEST_FILE_LOG !== "1" ? "silent" : "info";
