...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "maistro-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.MAISTRO_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "maistro-telegram-"));
  process.env.MAISTRO_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.MAISTRO_STATE_DIR = previous;
    }
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as MaistroConfig;
    const res = resolveTelegramToken(cfg);
