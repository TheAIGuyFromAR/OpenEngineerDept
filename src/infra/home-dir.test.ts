...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MAISTRO_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MAISTRO_HOME when set to ~", () => {
    const env = {
      MAISTRO_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MAISTRO_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MAISTRO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MAISTRO_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MAISTRO_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MAISTRO_HOME: "/srv/maistro-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/maistro-home")}/x`);
  });
