...
vi.mock("./maistro-root.js", () => ({
  resolveMaistroPackageRoot: vi.fn(async () => null),
  resolveMaistroPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveMaistroPackageRoot when available", async () => {
    const maistroRoot = await import("./maistro-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/maistro");
    (
      maistroRoot.resolveMaistroPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/maistro"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "maistro" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "maistro.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-maistro");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/maistro-bundle");
    (
      maistroRoot.resolveMaistroPackageRootSync as unknown as ReturnType<typeof vi.fn>
