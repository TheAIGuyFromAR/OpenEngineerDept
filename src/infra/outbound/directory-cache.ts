...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MaistroConfig | null = null;
  ...
  get(key: string, cfg: MaistroConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MaistroConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MaistroConfig): void {
