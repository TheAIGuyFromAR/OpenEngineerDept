...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MAISTRO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*maistro-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "maistro";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "maistro");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "maistro-gw");
  ...
  records.push(`_maistro-gw._tcp IN PTR ${instanceLabel}._maistro-gw._tcp`);
  records.push(`${instanceLabel}._maistro-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._maistro-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; maistro-content-hash: ${contentHash}\n${contentBody}`;
}
