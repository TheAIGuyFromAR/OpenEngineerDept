...
import { POSIX_MAISTRO_TMP_DIR, resolvePreferredMaistroTmpDir } from "./tmp-maistro-dir.js";

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `maistro-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMaistroTmpDir({
    accessSync,
...
describe("resolvePreferredMaistroTmpDir", () => {
  it("prefers /tmp/maistro when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MAISTRO_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/maistro when it does not exist but /tmp is writable", () => {
    const lstatSync = vi.fn(() => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MAISTRO_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/maistro when /tmp/maistro is not a directory", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => false,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("falls back to os.tmpdir()/maistro when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/maistro is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/maistro is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/maistro is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
