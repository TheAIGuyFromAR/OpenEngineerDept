...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MaistroConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MaistroConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: MaistroConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: MaistroConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MaistroConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MaistroConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MaistroConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
