...
export function triggerMaistroRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MAISTRO_SYSTEMD_UNIT,
        process.env.MAISTRO_PROFILE,
      );
  ...
  const label =
    process.env.MAISTRO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MAISTRO_PROFILE);
