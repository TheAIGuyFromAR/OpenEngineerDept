...
describe("update-startup", () => {
  ...
  let resolveMaistroPackageRoot: (typeof import("./maistro-root.js"))["resolveMaistroPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "MAISTRO_STATE_DIR");
    prevStateDir = process.env.MAISTRO_STATE_DIR;
    process.env.MAISTRO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveMaistroPackageRoot } = await import("./maistro-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveMaistroPackageRoot).mockResolvedValue("/opt/maistro");
