...
let ensureMaistroCliOnPath: typeof import("./path-env.js").ensureMaistroCliOnPath;

describe("ensureMaistroCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MAISTRO_PATH_BOOTSTRAPPED",
    "MAISTRO_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMaistroCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling maistro exists", () => {
    const tmp = abs("/tmp/maistro-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "maistro");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MAISTRO_PATH_BOOTSTRAPPED;

    ensureMaistroCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.MAISTRO_PATH_BOOTSTRAPPED = "1";
    ensureMaistroCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/maistro-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "maistro");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/maistro-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "maistro");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "maistro");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/maistro-path/case-linuxbrew");
