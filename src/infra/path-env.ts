...
function candidateBinDirs(opts: EnsureMaistroPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `maistro` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "maistro");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MAISTRO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "maistro"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `maistro` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMaistroCliOnPath(opts: EnsureMaistroPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MAISTRO_PATH_BOOTSTRAPPED)) {
    return;
  }
