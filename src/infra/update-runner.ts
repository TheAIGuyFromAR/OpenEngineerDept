...
const DEFAULT_PACKAGE_NAME = "maistro";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "maistro.mjs");
    ...
    const doctorStep = await runStep(
      step("maistro doctor", doctorArgv, gitRoot, { MAISTRO_UPDATE_IN_PROGRESS: "1" }),
    );
