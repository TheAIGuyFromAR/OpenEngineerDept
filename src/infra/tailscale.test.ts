...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.MAISTRO_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.MAISTRO_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.MAISTRO_TEST_TAILSCALE_BINARY;
    } else {
      process.env.MAISTRO_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
