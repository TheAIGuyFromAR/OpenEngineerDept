...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__maistro_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "maistro-root");
...
describe("resolveMaistroPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveMaistroPackageRootSync } = await import("./maistro-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "maistro");
    const pkgRoot = path.join(project, "node_modules", "maistro");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "maistro" }));

    expect(resolveMaistroPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "maistro");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "maistro.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "maistro" }));

    expect(resolveMaistroPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMaistroPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-maistro package roots", async () => {
    const { resolveMaistroPackageRootSync } = await import("./maistro-root.js");

    const pkgRoot = fx("not-maistro");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-maistro" }));

    expect(resolveMaistroPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveMaistroPackageRoot } = await import("./maistro-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "maistro" }));

    await expect(resolveMaistroPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
