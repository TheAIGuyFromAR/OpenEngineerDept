...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MAISTRO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Maistro";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Maistro\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.MAISTRO_MDNS_HOSTNAME?.trim() ||
    process.env.MAISTRO_MDNS_HOSTNAME?.trim() ||
    "maistro";
  const hostname =
    hostnameRaw
