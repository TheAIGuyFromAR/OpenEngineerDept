...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "maistro-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "maistro-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.maistro when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "maistro-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers MAISTRO_HOME over HOME", () => {
    vi.stubEnv("MAISTRO_HOME", "/srv/maistro-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/maistro-home"));
...
describe("shortenHomePath", () => {
  it("uses $MAISTRO_HOME prefix when MAISTRO_HOME is set", () => {
    vi.stubEnv("MAISTRO_HOME", "/srv/maistro-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/maistro-home")}/.maistro/maistro.json`)).toBe(
      "$MAISTRO_HOME/.maistro/maistro.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MAISTRO_HOME replacement when MAISTRO_HOME is set", () => {
    vi.stubEnv("MAISTRO_HOME", "/srv/maistro-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/maistro-home")}/.maistro/maistro.json`),
    ).toBe("config: $MAISTRO_HOME/.maistro/maistro.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/maistro")).toBe(path.resolve(os.homedir(), "maistro"));
  });
  ...
  it("prefers MAISTRO_HOME for tilde expansion", () => {
    vi.stubEnv("MAISTRO_HOME", "/srv/maistro-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/maistro")).toBe(path.resolve("/srv/maistro-home", "maistro"));
