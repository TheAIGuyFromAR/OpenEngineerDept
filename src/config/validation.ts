...
import type { MaistroConfig, ConfigValidationIssue } from "./types.js";
import { MaistroSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: MaistroConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MaistroConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MaistroSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MaistroConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MaistroConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MaistroConfig,
  };
}
