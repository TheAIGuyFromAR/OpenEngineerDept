...
import type { ConfigFileSnapshot } from "./types.maistro.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.maistro/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/maistro/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/maistro/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/maistro/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/maistro/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MaistroSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MaistroConfig";
    const hints = mapSensitivePaths(MaistroSchema, "", {});
