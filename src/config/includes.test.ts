...
const ETC_MAISTRO_DIR = path.join(ROOT_DIR, "etc", "maistro");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "maistro.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMaistroPath(...parts: string[]) {
  return path.join(ETC_MAISTRO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcMaistroPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "maistro.json"))).toEqual({
      shared: true,
    });
  });
});
