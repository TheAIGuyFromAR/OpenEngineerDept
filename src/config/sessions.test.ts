...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maistro-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-maistro",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-maistro#general");
  });
  ...
  it("derives session transcripts dir from MAISTRO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MAISTRO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.MAISTRO_STATE_DIR;
    process.env.MAISTRO_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.MAISTRO_STATE_DIR = prev;
      }
    }
  });
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.MAISTRO_STATE_DIR;
    const stateDir = path.resolve("/home/user/.maistro");
  ...
  it("resolves cross-agent paths when MAISTRO_STATE_DIR differs from stored paths", () => {
    const prev = process.env.MAISTRO_STATE_DIR;
    process.env.MAISTRO_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.MAISTRO_STATE_DIR;
    process.env.MAISTRO_STATE_DIR = path.resolve("/home/user/.maistro");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
