...
import type { MaistroConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareMaistroVersions } from "./version.js";
...
function coerceConfig(value: unknown): MaistroConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MaistroConfig;
}
...
function stampConfigVersion(cfg: MaistroConfig): MaistroConfig {
...
function warnIfConfigFromFuture(cfg: MaistroConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMaistroVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Maistro (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MaistroConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MaistroConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MaistroConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MaistroConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MaistroConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MAISTRO_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MAISTRO_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MAISTRO_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MAISTRO_WATCH_SESSION === "string" &&
        deps.env.MAISTRO_WATCH_SESSION.trim().length > 0
          ? deps.env.MAISTRO_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MAISTRO_WATCH_COMMAND === "string" &&
        deps.env.MAISTRO_WATCH_COMMAND.trim().length > 0
          ? deps.env.MAISTRO_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MAISTRO_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MAISTRO_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MAISTRO_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): MaistroConfig {
