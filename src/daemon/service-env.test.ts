...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MAISTRO_GATEWAY_PORT).toBe("18789");
    expect(env.MAISTRO_GATEWAY_TOKEN).toBe("secret");
    expect(env.MAISTRO_SERVICE_MARKER).toBe("maistro");
    expect(env.MAISTRO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MAISTRO_SERVICE_VERSION).toBe("string");
    expect(env.MAISTRO_SYSTEMD_UNIT).toBe("maistro-gateway.service");
    if (process.platform === "darwin") {
      expect(env.MAISTRO_LAUNCHD_LABEL).toBe("ai.maistro.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MAISTRO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MAISTRO_SYSTEMD_UNIT).toBe("maistro-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.MAISTRO_LAUNCHD_LABEL).toBe("ai.maistro.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maistro"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MAISTRO_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maistro-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MAISTRO_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maistro"));
  });

  it("uses MAISTRO_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MAISTRO_STATE_DIR: "/var/lib/maistro" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/maistro"));
  });

  it("expands ~ in MAISTRO_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MAISTRO_STATE_DIR: "~/maistro-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/maistro-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MAISTRO_STATE_DIR: "C:\\State\\maistro" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\maistro");
  });
});
