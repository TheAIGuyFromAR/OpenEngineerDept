...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.maistro.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", MAISTRO_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.maistro.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when MAISTRO_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.maistro.gateway.plist",
    );
  });

  it("uses profile-specific label when MAISTRO_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", MAISTRO_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.maistro.jbphoenix.plist",
    );
  });

  it("prefers MAISTRO_LAUNCHD_LABEL over MAISTRO_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      MAISTRO_PROFILE: "jbphoenix",
      MAISTRO_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from MAISTRO_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      MAISTRO_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty MAISTRO_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      MAISTRO_PROFILE: "myprofile",
      MAISTRO_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.maistro.myprofile.plist",
    );
  });
});
