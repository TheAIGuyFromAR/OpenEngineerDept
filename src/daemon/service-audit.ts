...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.MAISTRO_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service MAISTRO_GATEWAY_TOKEN does not match gateway.auth.token in maistro.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `maistro gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
