...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { MAISTRO_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAISTRO_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports MAISTRO_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { MAISTRO_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAISTRO_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { MAISTRO_SMOKE: "nope" })).toBe("qr");
  });
});
