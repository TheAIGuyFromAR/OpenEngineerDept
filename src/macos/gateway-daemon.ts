...
declare const __MAISTRO_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __MAISTRO_VERSION__ === "string" && __MAISTRO_VERSION__) ||
  process.env.MAISTRO_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `maistro --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.MAISTRO_GATEWAY_PORT ??
    process.env.MAISTRO_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.MAISTRO_GATEWAY_BIND ??
    process.env.MAISTRO_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (MAISTRO_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
