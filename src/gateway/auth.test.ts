...
describe("gateway auth", () => {
  it("resolves token/password from MAISTRO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          MAISTRO_GATEWAY_TOKEN: "env-token",
          MAISTRO_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy MAISTRO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          MAISTRO_GATEWAY_TOKEN: "legacy-token",
          MAISTRO_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
