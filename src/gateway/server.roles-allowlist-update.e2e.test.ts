...
const connectNodeClient = async (params: {
  ...
  const token = process.env.MAISTRO_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("MAISTRO_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".maistro", "restart-sentinel.json");
