...
export function resolveHooksConfig(cfg: MaistroConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MaistroConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-maistro-token"] === "string"
      ? req.headers["x-maistro-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
