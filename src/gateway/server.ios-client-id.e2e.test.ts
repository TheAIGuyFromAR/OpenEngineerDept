...
beforeAll(async () => {
  previousToken = process.env.MAISTRO_GATEWAY_TOKEN;
  process.env.MAISTRO_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.MAISTRO_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "maistro-ios", platform: "ios" },
  { clientId: "maistro-android", platform: "android" },
