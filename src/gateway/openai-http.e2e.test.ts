    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "maistro", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-maistro-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-maistro-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "maistro", messages: [{ role: "user", content: "hi" }] },
          {
            "x-maistro-agent-id": "beta",
            "x-maistro-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
