...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["MAISTRO_BASE_ENV"]);
    process.env.MAISTRO_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.MAISTRO_BASE_ENV ?? "") + "|" + (process.env.MAISTRO_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { MAISTRO_TEST_ENV: "ok" },
        },
      );
